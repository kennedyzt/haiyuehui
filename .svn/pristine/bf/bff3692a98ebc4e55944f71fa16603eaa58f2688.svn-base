            ...
            <tbody>
                <#list pageModel.records as t>
                    <tr>
                        <td width="10%">
                            <#if t.id!=0>
                                <input id="${t.id}" type="radio" <#if t.id==0></#if> name="check_box">
                                <a class="td_inner_a" href="<@url value='/businesspartner/edit?id=${t.id}' />" data-hover-tip="修改"><img src="<@static value='/icons/edit.png' />"></a>
                            </#if>
                        </td>
                        <td width="15%"><a href="<@url value='/businesspartner/get?id=${t.id}' />" data-hover-tip="查看详情">${t.partnerCode}</a></td>
