<#setting classic_compatible=true>
<@insert template="login/right">
    <div class="search_section">
        <div class="partner_text_div common_text_div">
            <div class="common_text_title">关键字</div>
            <div class="common_text_val"><input id="" class="input_text_common my_radius" data-click-tip="输入单据编号、供应商、联系人、商品、所有人关键字" placeholder="单据编号、供应商、联系人、商品、所有人"></div>
        </div>
        <div class="partner_text_div common_text_div">
            <div class="common_text_title">单据日期(从)</div>
            <div class="common_text_val"><input id="" class="input_text_common my_radius" data-type="date"></div>
        </div>
        <div class="partner_text_div common_text_div">
            <div class="common_text_title">单据日期(至)</div>
            <div class="common_text_val"><input id="" class="input_text_common my_radius" data-type="date"></div>
        </div>
        <div class="search_btn common_text_div">
            <button class="btn_common my_radius btn_search">搜索</button>
        </div>
        <div style="clear:both"></div>
    </div>
    <table>
        <thead>
            <tr>
                <td width="10%"><a class="td_head_a" onclick="deleteDraftReceivePO()" data-hover-tip="删除"><img src="<@static value='/icons/delete.png' />"></a></td>
                <td width="10%">单据编号</td>
                <td width="10%">单据日期</td>
                <td width="10%">入库仓库</td>
                <td width="10%">供应商</td>
                <td width="10%">名称</td>
                <td width="10%">联系人</td>
                <td width="10%">移动电话</td>
                <td width="20%">单据摘要</td>
            </tr>
        </thead>
    </table>
    <div class="data_grid">
        <table>
            <tbody>
                <tr>
                    <td width="10%">
                        <input id="" type="radio" name="check_box">
                        <a class="td_inner_a" href="<@url value='#' />" data-hover-tip="修改"><img src="<@static value='/icons/edit.png' />"></a>
                    </td>
                    <td width="10%"><a href="<@url value='#' />" data-hover-tip="查看详情">4542121212112</a></td>
                    <td width="10%"></td>
                    <td width="10%"></td>
                    <td width="10%"></td>
                    <td width="10%"></td>
                    <td width="10%"></td>
                    <td width="10%"></td>
                    <td width="20%"></td>
                </tr>
            </tbody>
        </table>
    </div>
    <div class="order_btns_l common_text_div">
        <button id="goback_btn" class="btn_common my_radius">返回</button>
    </div>
</@insert>